...
colorPicker.addEventListener('input', e => { color = e.target.value; save('canvas_brush_color', color); });
undoBtn.onclick = () => { if (undoIdx > 0) { undoIdx--; restoreUndo(); } };
redoBtn.onclick = () => { if (undoIdx < undoStack.length - 1) { undoIdx++; restoreUndo(); } };
clearBtn.onclick = () => {
  const dpr = devicePixelRatio || 1;
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  const r = container.getBoundingClientRect();
...
slider.addEventListener('input', e => {
  brushSize = +e.target.value; sliderVal.textContent = brushSize;
  setCursorSize(brushSize); save('canvas_brush_size', brushSize);
});
...
document.addEventListener('keydown', e => {
  if (e.key === 'z' && (e.ctrlKey || e.metaKey) && e.shiftKey) { e.preventDefault(); redoBtn.click(); }
  else if (e.key === 'z' && (e.ctrlKey || e.metaKey)) { e.preventDefault(); undoBtn.click(); }
...
function setBg(bg, repaint) {
  ...
  // Repaint canvas: swap old bg color pixels to new bg color
...
function updateGrid() {
  if (gridOn) {
    const color = canvasBg === '#000000' ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)';
...
async function loadPrefs() {
  try {
    ...
    if (p.canvas_brush_size) { brushSize = +p.canvas_brush_size; slider.value = brushSize; sliderVal.textContent = brushSize; setCursorSize(brushSize); }
    if (p.canvas_brush_color) { color = p.canvas_brush_color; colorPicker.value = color; }
...
// Use requestAnimationFrame to ensure layout is computed before init
requestAnimationFrame(() => {
  init();
  // Also set up ResizeObserver for future resizes
